...
d3.csv('datasets/izone_videos.csv').then(function(data){
  ...
  zero.html('<p>For IZ*ONE, among the top 50 most-viewed videos on Youtube, the highest one has <strong style="font-size: 32px;">' + data[0].viewCount +' </strong>views, which is the lowest among all the four groups; the lowest one has <strong style="font-size: 32px;">' + data[49].viewCount +'</strong> views.</p>');


  let one = d3.select("#one").append("div");

  one.html('<p>Among the top 50 most-viewed videos, the highest like/dislike ratio is <strong style="font-size: 32px;">' + d3.max(ratio) +' </strong>; the lowest one is <strong style="font-size: 32px;">' + d3.min(ratio) +'</strong>. </br> The average ratio is <strong style="font-size: 32px;">' + formatDecimal(d3.mean(decimals)) +'</strong>.</p>');

  let two = d3.select("#two").append("div");

  two.html('<p>The Category <strong style="font-size: 32px;">"Music"</strong> is used most frequently. </br>Categories belong to all kinds of <strong style="font-size: 32px;">"Video Game" </strong>is unique to this group.</p>');
  ...
  function Scroll2(){
    ...
    rect
      .transition()
      .attr('x', w/3 + 20)
      .attr('y', function(d, i){return (30 + i *35)+'px'})
    ...
    number
      ...
      .attr("fill", "#f2f0eb")
      .attr('x', w/3 + 25)
      .attr('y', function(d, i){return (47 + i *35)+'px'})
    ;
